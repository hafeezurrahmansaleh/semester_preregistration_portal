...
def updateRegistration(request, sid,sem):
    ...
    # courses = CoursePreRegistration.objects.filter(semester=semester, student=student).values('course__courseCode', 'course__courseTitle','course__courseCredit', 'course_id', 'section','semester__semesterTitle')
    regcourses=CoursePreRegistration.objects.filter(semester=semester, student=student).values(
        'id', 'course__courseCode', 'course__courseTitle','course__courseCredit',
        'course__totalSection', 'course_id', 'section','semester__semesterTitle',
        'student__stName','student__stID')
    print(regcourses)
...
def saveCourse(request):
    ...
    if(CoursePreRegistration.objects.filter(course_id=cid,student_id=sid, semester_id=semid).exists()):
        ...
        update.save()
        try:
            getreg = CoursePreRegistration.objects.get(course_id=cid, student_id=sid,
                                                    semester_id=semid)
            regid = getreg.id
        except:
            regid=-1
...
def findCourse(request):
    cid = request.POST['cid']
    courses = Courses.objects.filter(pk=cid).values('id','courseCode','courseTitle',
                                                    'courseCredit','totalSection')
